...
class Vector3 {
   public:
    ...
    HD Vector3(float n) : x(n), y(n), z(n) {}
    ...
    HD Vector3 cross_product(const Vector3& second) const {
        return Vector3(y * second.z - z * second.y, z * second.x - x * second.z,
                       x * second.y - y * second.x);
    }

    HD float distance(const Vector3& other) const {
    ...
    HD Vector3 operator+(const Vector3& second) const {
        return Vector3(this->x + second.x, this->y + second.y,
                       this->z + second.z);
    }
    ...
    HD Vector3 operator*(Vector3 const& other) const {
