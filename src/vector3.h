...
class Vector3 {
   public:
    ...
    HD Vector3(const Vector3& begin, const Vector3& end) : x(end.x - begin.x), y(end.y - begin.y), z(end.z - begin.z) {}

    HD float scalar_product(const Vector3& second) const { return x * second.x + y * second.y + z * second.z; }
    ...
    HD Vector3 operator*(float scalar) const { return Vector3(this->x * scalar, this->y * scalar, this->z * scalar); }
};
